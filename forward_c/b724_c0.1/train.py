...
class PINN:
    ...
    def __init__(self):
        ...
        # 边界处的u值
        u_ic = torch.tensor([[-0.5081554489307964,0.23058975287891456,-0.30553946998216036,-0.23594333694632602]])  # t=0边界处采用第一类边界条件u=-sin(pi*x)
        ...
        self.lbfgs = torch.optim.LBFGS(
            ...
            tolerance_change=1e-15, #1.0 * np.finfo(float).eps,
            line_search_fn="strong_wolfe",
        )

        # 设置adam优化器
        self.adam = torch.optim.Adam(self.model.parameters(),lr=0.0001)#3e-3
    ...
    def loss_func(self):
        ...
        # 最终的loss由两项组成
        loss = loss_equation0 + 0.6*loss_boundary
